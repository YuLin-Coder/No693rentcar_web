...
<meta charset="UTF-8">
<meta name="viewport"
	content="width=device-width, EmployeeDate-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
					...
					<div class="main">
						...
						<div class="form-box">
							<div class="row">
								<div class="col-5">
									<form id="saveForm">
									<input type="hidden" name="id" value="${EmployeeDate.id }"> 

											<div class="form-group">
											<label>姓名</label> <input type="text" name="name" value="${EmployeeDate.name }" readonly
												class="form-control"> <small
												class="form-text text-muted"></small>
										</div>

										<div class="form-group">
											<label>员工编号</label> <input type="text" name="eno" value="${EmployeeDate.eno }"
												class="form-control"> <small
												class="form-text text-muted"></small>
										</div>

										<div class="form-group">
											<label>密码</label> <input type="password" name="pwd" value="${EmployeeDate.pwd }"
												class="form-control"> <small
												class="form-text text-muted"></small>
										</div>
											...
											<label>性别</label> <select name="sex" class="form-control">
											           <option value="男" <c:if test="${'男' ==  EmployeeDate.sex }">selected</c:if>  >男</option>
											           <option value="女"  <c:if test="${'女' ==  EmployeeDate.sex }">selected</c:if> >女</option>
											</select> <small class="form-text text-muted"></small>
										</div>
										

 										 <input type="hidden" name="photo" class="photo" value="${EmployeeDate.photo }">
										   <div class="form-group">
						...
						<div class="submit-box">
							...
							<a type="button" href="EmployeeList" class="btn btn-outline-secondary">返回</a>
		...
		$("#save").click(function() {
			$.ajax({
				...
				success : function(e) {
					if (e) {
						alert("修改成功！");
						window.parent.location.href = "EmployeeList";
					} else {
						alert("修改失败！");
					}
				}
