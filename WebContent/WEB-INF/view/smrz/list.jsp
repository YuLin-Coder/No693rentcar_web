...
<body>
								...
								<thead>
									<tr>
								    	 ...
								    	 <td scope="col">身份证号</td>
					                      <td scope="col">身份证正面</td>
					                      <td scope="col">身份证反面</td>
					                      <td scope="col">状态</td>
										<th scope="col">操作</th>
									</tr>
								</thead>
								<tbody>
									...
									<c:forEach items="${pageInfo.list}" var="data">
										<tr>
											...
											<td>${data.cno }</td>
											<td><img src="/images/${data.photo1}" id="preview_img" style="width:50px; height:50px; "></td>		
											
											<td><img src="/images/${data.photo2}" id="preview_img" style="width:50px; height:50px; "></td>		
                                   ...
                                   <td><input id="${data.id}" value="${data.id}"
												type="hidden" class="weui-input" />
												 <c:if test="${data.state == '0' }">
												  <a class="btn btn-primary btn-sm" type="button" onclick="updateSmrzStatus(${data.id},1,${data.uid})">通过</a>
												  <a class="btn btn-warning btn-sm" type="button" onclick="updateSmrzStatus(${data.id},2,${data.uid})">拒绝</a>
												  </c:if>
 ...
 -->
	...
	function updateSmrzStatus(id,state,uid){
		if(confirm("确定要审批这条数据吗?")){
	 		 　 $.ajax({
					url:"updateSmrz",
				  	data:{"id": id,"state":state,"uid":uid},
					type:"post",
					dataType:"json",
